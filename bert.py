...
from data import Dataset, Tree, Field, RawField, ChartField, InputFeatures, bertify
import argparse
...
class Model(nn.Module):
    ...
    def embed(self, words, feats):
        ...
        input_ids, segment_ids, input_mask = ext_words
        with torch.no_grad():
            word_embed, _ = self.word_embed(input_ids, token_type_ids=segment_ids,
                                            attention_mask=input_mask, output_all_encoded_layers=False)
        # forward(self, subwords, bert_lens, bert_mask):
...
if __name__ == "__main__":
    ...
    WORD = Field('words', pad='[PAD]', unk='[UNK]', bos='[CLS]', eos='[SEP]', lower=True)
    TAG = Field('tags', bos="[CLS]", eos='[EOS]')
    TREE = RawField('trees')
    global CHART
